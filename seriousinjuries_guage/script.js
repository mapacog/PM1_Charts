...
d3.csv(`PM1_Viewer.csv?ts=${Date.now()}`)
  .then(function (rows) {
    ...
    if (!observedRow) {
      console.error("No observed serious-injury rolling-average row found in PM1_Viewer.csv");
      return;
    }

    const observedYear = mainYear;
    const observed   = toNum(observedRow["Serious Injuries (5-yr avg)"]);
    const targetPast =
      toNum(observedRow["Serious Injury Target (Current)"]) ??
      toNum(observedRow["Serious Injury Target (Past)"]);

    if (observed == null || targetPast == null) {
      console.error(`Missing observed or past-target serious injuries for ${observedYear}.`);
      return;
    }

    document.title = `Serious Injuries (5-Year Rolling Avg, ${observedYear})`;
