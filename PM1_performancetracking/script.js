...
d3.csv(`PM1_Viewer.csv?ts=${Date.now()}`)
  .then(function (rows) {
    ...
    const baselineYear = 2023; // Previous adopted baseline
    const projYear = 2025;     // Projected & Observed main year
    const targetYear = 2027;   // Current state target year
    ...
    if (!baselineRow || !projRow || !targetRow) {
      console.error("Missing one of the required years: 2023 (baseline), 2025 (proj/obs), 2027 (targets).");
      return;
    }
